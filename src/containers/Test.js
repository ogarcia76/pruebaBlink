...
function Test() {
  ...
  const { tests, setTest } = useContext( TestsContext ) 
  const [responses, setResponses] = useState( tests.test );
  ...
  const saveResponses = () => {
    setTest({ ...responses, done: true });
    setResponses({ ...responses, done: true });
    navigate(`/`);
  }
